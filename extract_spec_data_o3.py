...
def extract_parameters(config_file):
    ...
    with open(config_file, 'r') as file:
        ...
        for i, line in enumerate(lines):
            if line.startswith('fetchToDecodeDelay'):
                params['fetchToDecodeDelay'] = line.strip().split('=')[1].strip()
            elif line.startswith('decodeToRenameDelay'):
                params['decodeToRenameDelay'] = line.strip().split('=')[1].strip()
            elif line.startswith('renameToIEWDelay'):
                params['renameToIEWDelay'] = line.strip().split('=')[1].strip()
...
def process_simulations(main_folder):
        ...
        if os.path.isdir(sim_path):
            ...
            if os.path.exists(config_file) and os.path.exists(stats_file):
                ...
                simulation_data = {
                    ...
                    'fetchToDecodeDelay': params.get('fetchToDecodeDelay', ''),
                    'decodeToRenameDelay': params.get('decodeToRenameDelay', ''),
                    'renameToIEWDelay': params.get('renameToIEWDelay', ''),
    ...
    simulations.sort(key=lambda x: (
        x['simulation_name'],
        x['clock'],
        int(x['fetchToDecodeDelay']),
        int(x['decodeToRenameDelay']),
        int(x['renameToIEWDelay'])
    ))
    ...
    with open(csv_filename, 'w', newline='') as csvfile:
        fieldnames = ['simulation_name', 'fetchToDecodeDelay', 'decodeToRenameDelay', 'renameToIEWDelay', 'clock', 'cycle_count']
